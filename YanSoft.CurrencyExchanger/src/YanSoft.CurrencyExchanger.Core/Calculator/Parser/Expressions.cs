...
namespace YanSoft.CurrencyExchanger.Core.Calculator.Parser
{
    ...
    {
        private static Dictionary<string, object> environment;
        /// <summary>
        /// 
        /// </summary>
        ...
        public bool HasValue
        {
            get
            {
                return environment.ContainsKey(identifier.ToLower());
            }
        }
    ...
    /// </summary>
    public sealed class SgnExpression : UnaryExpression
    {
        public SgnExpression(IExpression child)
            : base(child)
        {
        }
        ...
        public override IExpression Simplify()
        {
            ...
            if (childConst != null)
                // child is constant;  just evaluate it;
                return new ConstantExpression(Math.Sign(childConst.Value));

            return new SgnExpression(newChild);
        }

        public override string ToString()
        {
            return "sgn(" + child.ToString() + ")";
        }
    }
    ...
    /// </summary>
    public sealed class AbsExpression : UnaryExpression
    {
        public AbsExpression(IExpression child)
            : base(child)
        {
        }
        ...
        public override IExpression Differentiate(string byVar)
        {
            //    f(x)   = abs( g(x) );
            // d( f(x) ) = sgn( g(x) ) * d( g(x) );
            return new MultExpression(new SgnExpression(child), child.Differentiate(byVar));
        }
        ...
        public override IExpression Simplify()
        {
            ...
            if (childConst != null)
                // child is constant;  just evaluate it;
                return new ConstantExpression(Math.Abs(childConst.Value));
    ...
    /// Evaluates the absolute value of an expression.
    /// </summary>
    public sealed class SqrtExpression : UnaryExpression
    {
        public SqrtExpression(IExpression child)
            : base(child)
        {
        }
        ...
        {
            //    f(x)   = sqrt( g(x) );
            // d( f(x) ) = ( 0.5/sqrt(g(x)) ) * d( g(x) );
            return new ExpExpression(child, new ConstantExpression(0.5));

            //// or if you want to do this backwards ;-)
            //return new MultExpression(
            //    new ConstantExpression(0.5), 
            //    new MultExpression(new SqrtExpression(child), child.Differentiate(byVar))
            //    );
        }
        ...
        public override IExpression Simplify()
        {
            ...
            if (childConst != null)
                // child is constant;  just evaluate it;
                return new ConstantExpression(Math.Sqrt(childConst.Value));

            return new SqrtExpression(newChild);
        }
    ...
    /// <summary>
    /// Evaluates the sine of an expression.
    /// </summary>
    public sealed class SineExpression : UnaryExpression
    {
        public SineExpression(IExpression child)
            : base(child)
        {
        }
    ...
    /// <summary>
    /// Evaluates the cosine of an expression.
    /// </summary>
    public sealed class CosineExpression : UnaryExpression
    {
        public CosineExpression(IExpression child)
            : base(child)
        {
        }
    ...
    /// <summary>
    /// Evaluates the tangent of an expression.
    /// </summary>
    public sealed class TangentExpression : UnaryExpression
    {
        public TangentExpression(IExpression child)
            : base(child)
        {
        }
    ...
    /// <summary>
    /// Evaluates the hyperbolic sine of an expression.
    /// </summary>
    public sealed class HyperbolicSineExpression : UnaryExpression
    {
        public HyperbolicSineExpression(IExpression child)
            : base(child)
        { }
        ...
        {
            //      f(x) = sinh( g(x) );
            // d( f(x) ) = cosh( g(x) ) * d( g(x) );
            return new MultExpression(
                new HyperbolicCosineExpression(child),
                child.Differentiate(byVar));
        }
        ...
        public override IExpression Simplify()
        {
            ...
            if (childConst != null)
                // child is constant;  just evaluate it;
                return new ConstantExpression(Math.Sinh(childConst.Value));

            return new HyperbolicSineExpression(newChild);
        }
    ...
    /// <summary>
    /// Evaluates the hyperbolic cosine of an expression.
    /// </summary>
    public sealed class HyperbolicCosineExpression : UnaryExpression
    {
        public HyperbolicCosineExpression(IExpression child)
            : base(child)
        { }
        ...
        public override IExpression Differentiate(string byVar)
        {
            //      f(x) = sinh( g(x) );
            // d( f(x) ) = cosh( g(x) ) * d( g(x) );
            return new MultExpression(new HyperbolicSineExpression(child), child.Differentiate(byVar));
        }
        ...
        public override IExpression Simplify()
        {
            ...
            if (childConst != null)
                // child is constant;  just evaluate it;
                return new ConstantExpression(Math.Cosh(childConst.Value));

            return new HyperbolicCosineExpression(newChild);
        }
    ...
    /// <summary>
    /// Evaluates the hyperbolic tangent of an expression.
    /// </summary>
    public sealed class HyperbolicTangentExpression : UnaryExpression
    {
        public HyperbolicTangentExpression(IExpression child)
            : base(child)
        {
        }
        ...
        public override IExpression Differentiate(string byVar)
        {
            //      f(x) = tan( g(x) );
            // d( f(x) ) = cos^-2( g(x) ) * d( g(x) );
            return new MultExpression(new ExpExpression(new HyperbolicCosineExpression(child),
                                                          new ConstantExpression(-2)),
                                       child.Differentiate(byVar));
        }
        ...
        public override IExpression Simplify()
        {
            ...
            if (childConst != null)
                // child is constant;  just evaluate it;
                return new ConstantExpression(Math.Tanh(childConst.Value));

            return new HyperbolicTangentExpression(newChild);
        }
    ...
    /// <summary>
    /// Evaluates the log10 of an expression.
    /// </summary>
    public sealed class LogExpression : UnaryExpression
    {
        public LogExpression(IExpression child)
            : base(child)
        { }
        ...
        public override IExpression Differentiate(string byVar)
        {
            //      f(x) = log( g(x) );
            // d( f(x) ) = d( g(x) ) / g(x);
            return new DivExpression(child.Differentiate(byVar), child);
        }
        ...
        public override IExpression Simplify()
        {
            ...
            {
                // child is constant;  just evaluate it;
                return new ConstantExpression(Math.Log10(childConst.Value));
            }
    ...
    /// <summary>
    /// Evaluates the natural log of an expression.
    /// </summary>
    public sealed class LnExpression : UnaryExpression
    {
        public LnExpression(IExpression child)
            : base(child)
        { }
        ...
        public override IExpression Simplify()
        {
            ...
            if (childConst != null)
                // child is constant;  just evaluate it;
                return new ConstantExpression(Math.Log(childConst.Value));
    ...
    {
        public MultExpression(IExpression left, IExpression right)
            : base(left, right)
        {
        }
    ...
    {
        public ExpExpression(IExpression left, IExpression right)
            : base(left, right)
        {
        }
